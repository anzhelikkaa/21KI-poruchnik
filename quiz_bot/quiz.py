...
def show_topics(bot, chat_id, user):
...
def start_quiz(bot, chat_id, topic):
    user_data[chat_id] = {
        'topic': topic,
        'current_question': 0,
        'score': 0,
...
def handle_quiz_decision(bot, chat_id, decision, user):
    if decision == "Почати вікторину":
        selected_topic = user_data[chat_id]['selected_topic']
        start_quiz(bot, chat_id, selected_topic)
    elif decision == "Повернутися до вибору теми":
        show_topics(bot, chat_id, user)

def handle_topic_selection(bot, chat_id, selected_topic):
...
@log_time
def calculate_results(bot, chat_id):
    user = user_data[chat_id]
